...
mod tests {
    ...
    fn model_filter_works() {
        let db = test_db();
        record_inference_cost(&db, "claude-4", "anthropic", 100, 50, 0.01, None, false, None, None, false).unwrap();
        record_inference_cost(&db, "gpt-4", "openai", 200, 100, 0.02, None, false, None, None, false).unwrap();
    ...
    fn all_cached_full_rate() {
        let db = test_db();
        record_inference_cost(&db, "m1", "p1", 100, 50, 0.01, None, true, None, None, false).unwrap();
        record_inference_cost(&db, "m1", "p1", 100, 50, 0.01, None, true, None, None, false).unwrap();
    ...
    fn zero_tokens_in_no_division_by_zero() {
        let db = test_db();
        record_inference_cost(&db, "m1", "p1", 0, 50, 0.01, None, false, None, None, false).unwrap();
    ...
    fn zero_tokens_out_no_division_by_zero() {
        let db = test_db();
        record_inference_cost(&db, "m1", "p1", 1000, 0, 0.01, None, false, None, None, false).unwrap();
    ...
    fn no_cached_zero_rate() {
        let db = test_db();
        record_inference_cost(&db, "m1", "p1", 100, 50, 0.01, None, false, None, None, false).unwrap();
        record_inference_cost(&db, "m1", "p1", 100, 50, 0.01, None, false, None, None, false).unwrap();
    ...
    fn multiple_models_identifies_most_efficient() {
        ...
        record_inference_cost(&db, "m1", "p1", 1000, 500, 0.01, None, false, None, None, false).unwrap();
        // m2: 100 in, 200 out -> density ~2.0  (more efficient)
        record_inference_cost(&db, "m2", "p2", 100, 200, 0.005, None, false, None, None, false).unwrap();
    ...
    fn build_user_profile_with_data() {
        ...
        .unwrap();
        record_inference_cost(&db, "gpt-4", "openai", 500, 200, 0.01, None, false, None, None, false).unwrap();
    ...
    fn report_cost_attribution_all_history() {
        let db = test_db();
        record_inference_cost(&db, "m1", "p1", 1000, 500, 0.03, None, false, None, None, false).unwrap();
