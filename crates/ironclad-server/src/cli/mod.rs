...
pub fn init_theme(color_flag: &str, theme_flag: &str, no_draw: bool, nerdmode: bool) {
    ...
    let t = if nerdmode { t.with_nerdmode(true) } else if no_draw { t.with_draw(false) } else { t };
    let _ = THEME.set(t);
}
...
pub(crate) fn colors() -> (&'static str, &'static str, &'static str, &'static str, &'static str, &'static str, &'static str, &'static str, &'static str) {
    let t = theme();
    (t.dim(), t.bold(), t.accent(), t.success(), t.warn(), t.error(), t.info(), t.reset(), t.mono())
}

pub(crate) fn icons() -> (&'static str, &'static str, &'static str, &'static str, &'static str) {
    let t = theme();
    (t.icon_ok(), t.icon_action(), t.icon_warn(), t.icon_detail(), t.icon_error())
}
...
impl IroncladClient {
    ...
    fn check_connectivity_hint(e: &dyn std::error::Error) {
        ...
        let msg = format!("{e:?}");
        if msg.contains("Connection refused") || msg.contains("ConnectionRefused") || msg.contains("ConnectError") || msg.contains("connect error") {
            eprintln!();
            eprintln!("  {WARN} Is the Ironclad server running? Start it with: {BOLD}ironclad serve{RESET}");
        }
    }
}
...
pub(crate) fn truncate_id(id: &str, len: usize) -> String {
    if id.len() > len { format!("{}...", &id[..len]) } else { id.to_string() }
}
...
pub(crate) fn table_header(headers: &[&str], widths: &[usize]) {
    let (DIM, BOLD, ACCENT, GREEN, YELLOW, RED, CYAN, RESET, MONO) = colors();
    let (OK, ACTION, WARN, DETAIL, ERR) = icons();
    let cells: Vec<String> = headers.iter().zip(widths).map(|(h, w)| format!("{BOLD}{h:<width$}{RESET}", width = w)).collect();
...
pub(crate) fn table_row(cells: &[String], widths: &[usize]) {
    let (DIM, BOLD, ACCENT, GREEN, YELLOW, RED, CYAN, RESET, MONO) = colors();
    let (OK, ACTION, WARN, DETAIL, ERR) = icons();
    let formatted: Vec<String> = cells.iter().zip(widths).map(|(c, w)| {
        let visible_len = strip_ansi_len(c);
        if visible_len >= *w { c.clone() } else { format!("{c}{}", " ".repeat(w - visible_len)) }
    }).collect();
...
pub(crate) fn print_json_section(val: &Value, indent: usize) {
    ...
    match val {
        Value::Object(map) => {
            for (k, v) in map {
                match v {
                    Value::Object(_) => { eprintln!("{pad}{DIM}{k}:{RESET}"); print_json_section(v, indent + 2); }
                    Value::Array(arr) => {
                        let items: Vec<String> = arr.iter().map(|i| format_json_val(i).to_string()).collect();
...
pub(crate) fn format_json_val(v: &Value) -> String {
    ...
    match v {
        ...
        Value::Bool(b) => if *b { format!("{GREEN}{b}{RESET}") } else { format!("{YELLOW}{b}{RESET}") },
...
pub(crate) fn urlencoding(s: &str) -> String {
    s.replace(' ', "%20").replace('&', "%26").replace('=', "%3D").replace('#', "%23")
}
...
mod tests {
    use super::*;
    #[test] fn client_construction() { let c = IroncladClient::new("http://localhost:18789").unwrap(); assert_eq!(c.base_url, "http://localhost:18789"); }
    #[test] fn client_strips_trailing_slash() { let c = IroncladClient::new("http://localhost:18789/").unwrap(); assert_eq!(c.base_url, "http://localhost:18789"); }
    #[test] fn truncate_id_short() { assert_eq!(truncate_id("abc", 10), "abc"); }
    #[test] fn truncate_id_long() { assert_eq!(truncate_id("abcdefghijklmnop", 8), "abcdefgh..."); }
    #[test] fn status_badges() { assert!(status_badge("ok").contains("ok")); assert!(status_badge("dead").contains("dead")); assert!(status_badge("foo").contains("foo")); }
    #[test] fn strip_ansi_len_works() { assert_eq!(strip_ansi_len("hello"), 5); assert_eq!(strip_ansi_len("\x1b[32mhello\x1b[0m"), 5); }
    #[test] fn urlencoding_encodes() { assert_eq!(urlencoding("hello world"), "hello%20world"); assert_eq!(urlencoding("a&b=c#d"), "a%26b%3Dc%23d"); }
    #[test] fn format_json_val_types() { assert!(format_json_val(&Value::String("test".into())).contains("test")); assert!(format_json_val(&serde_json::json!(42)).contains("42")); assert!(format_json_val(&Value::Null).contains("null")); }
    #[test] fn which_binary_finds_sh() { assert!(which_binary("sh").is_some()); }
    #[test] fn which_binary_returns_none_for_nonsense() { assert!(which_binary("__ironclad_nonexistent_binary_98765__").is_none()); }
}
