...
pub async fn cmd_memory(url: &str, tier: &str, session_id: Option<&str>, query: Option<&str>, limit: Option<i64>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match tier {
        "working" => {
            ...
            let data = c.get(&format!("/api/memory/working/{sid}")).await.map_err(|e| { IroncladClient::check_connectivity_hint(&*e); e })?;
            ...
            match entries {
                Some(arr) if !arr.is_empty() => {
                    ...
                    eprintln!(); eprintln!("    {DIM}{} entries{RESET}", arr.len());
                }
        ...
        "episodic" => {
            ...
            let data = c.get(&format!("/api/memory/episodic?limit={lim}")).await.map_err(|e| { IroncladClient::check_connectivity_hint(&*e); e })?;
            ...
            match entries {
                Some(arr) if !arr.is_empty() => {
                    ...
                    eprintln!(); eprintln!("    {DIM}{} entries (limit: {lim}){RESET}", arr.len());
                }
        ...
        "semantic" => {
            ...
            let data = c.get(&format!("/api/memory/semantic/{category}")).await.map_err(|e| { IroncladClient::check_connectivity_hint(&*e); e })?;
        ...
        "search" => {
            ...
            let data = c.get(&format!("/api/memory/search?q={}", urlencoding(q))).await.map_err(|e| { IroncladClient::check_connectivity_hint(&*e); e })?;
            ...
            match results {
                Some(arr) if !arr.is_empty() => {
                    ...
                    eprintln!(); eprintln!("    {DIM}{} results{RESET}", arr.len());
                }
                _ => empty_state("No results found"),
            }
        }
        _ => return Err(format!("unknown memory tier: {tier}. Use: working, episodic, semantic, search").into()),
    }
    eprintln!();
    Ok(())
}
