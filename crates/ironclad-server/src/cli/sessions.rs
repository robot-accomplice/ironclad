...
pub async fn cmd_sessions_list(url: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let data = c.get("/api/sessions").await.map_err(|e| { IroncladClient::check_connectivity_hint(&*e); e })?;
    heading("Sessions");
    ...
    match sessions {
        Some(arr) if !arr.is_empty() => {
            ...
            for s in arr {
                ...
                table_row(&[format!("{MONO}{id}{RESET}"), agent, format!("{DIM}{created}{RESET}"), format!("{DIM}{updated}{RESET}")], &widths);
            }
            eprintln!();
...
pub async fn cmd_session_detail(url: &str, id: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (OK, ACTION, WARN, DETAIL, ERR) = icons();
    let c = IroncladClient::new(url)?;
    let session = c.get(&format!("/api/sessions/{id}")).await.map_err(|e| { IroncladClient::check_connectivity_hint(&*e); e })?;
    ...
    match msgs {
        Some(arr) if !arr.is_empty() => {
            ...
            for m in arr {
                ...
                let role_color = match role { "user" => CYAN, "assistant" => GREEN, "system" => YELLOW, _ => DIM };
                let short_time = if time.len() > 19 { &time[11..19] } else { time };
                eprintln!("    {role_color}\u{25b6}{RESET} {role_color}{BOLD}{role}{RESET} {DIM}{short_time}{RESET}");
                for line in content.lines() { eprintln!("      {line}"); }
                eprintln!();
            }
        }
        _ => { eprintln!(); empty_state("No messages in this session"); }
    }
    eprintln!();
    Ok(())
}

pub async fn cmd_session_create(url: &str, agent_id: &str) -> Result<(), Box<dyn std::error::Error>> {
    let (DIM, BOLD, ACCENT, GREEN, YELLOW, RED, CYAN, RESET, MONO) = colors();
    let (OK, ACTION, WARN, DETAIL, ERR) = icons();
    let c = IroncladClient::new(url)?;
    let body = serde_json::json!({ "agent_id": agent_id });
    let result = c.post("/api/sessions", body).await.map_err(|e| { IroncladClient::check_connectivity_hint(&*e); e })?;
...
pub async fn cmd_session_export(base_url: &str, session_id: &str, format: &str, output: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
    let (DIM, BOLD, ACCENT, GREEN, YELLOW, RED, CYAN, RESET, MONO) = colors();
    let (OK, ACTION, WARN, DETAIL, ERR) = icons();
    ...
    let content = match format {
        ...
        "html" => {
            ...
            html.push_str("h1{color:#8b5cf6}.msg{margin:16px 0;padding:12px 16px;border-radius:8px}");
            ...
            for msg in &messages {
                ...
                let content = msg.get("content").and_then(|v| v.as_str()).unwrap_or("");
                let ts = msg.get("created_at").and_then(|v| v.as_str()).unwrap_or("");
                let class = match role { "user" => "user", "assistant" => "assistant", "system" => "system", _ => "msg" };
    ...
    match output {
        Some(path) => { std::fs::write(path, &content)?; eprintln!("  {OK} Exported to {path}"); }
        None => print!("{content}"),
    }
    Ok(())
}
