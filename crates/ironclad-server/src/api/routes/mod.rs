...
"#;
    ...
    async fn memory_working_by_session_returns_seeded_entries() {
        let state = test_state();
        ironclad_db::memory::store_working(&state.db, "sess-1", "observation", "the sky is blue", 3)
            .unwrap();
    ...
    async fn policy_audit_returns_seeded_decisions() {
        ...
        assert!(decisions.iter().any(|d| d["tool_name"] == "shell_exec" && d["decision"] == "deny"));
        assert!(decisions.iter().any(|d| d["tool_name"] == "read_file" && d["decision"] == "allow"));
    }
    ...
    async fn tool_audit_returns_seeded_calls() {
        ...
        let session_id =
            ironclad_db::sessions::create_new(&state.db, "test-agent", None).unwrap();
    ...
    async fn malformed_json_returns_json_error_body() {
        ...
        assert_eq!(resp.status(), StatusCode::BAD_REQUEST);
        let body = json_body(resp).await;
        assert!(body["error"].is_string(), "error response must be JSON with 'error' field");
    }
    ...
    async fn session_list_respects_limit_parameter() {
        ...
        for i in 0..5 {
            ironclad_db::sessions::rotate_agent_session(
                &state.db,
                &format!("agent-{i}"),
            )
            .unwrap();
        ...
        let resp = app
            .oneshot(
                Request::builder()
                    ...
                    .uri("/api/roster/TestBot/model")
                    .header("content-type", "application/json")
                    .body(Body::from(r#"{"model":"anthropic/claude-sonnet-4-20250514"}"#))
                    .unwrap(),
            )
            .await
            .unwrap();
