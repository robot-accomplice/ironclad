...
pub async fn submit_revenue_tax_task(
    ...
    let claimed = ironclad_db::revenue_tax_tasks::claim_revenue_tax_submission(
        &state.db,
        &opportunity_id,
    )
    .map_err(|e| internal_err(&e))?;
    if !claimed {
        return Err(bad_request(
            "revenue tax task is not available for submission; \
    ...
    .await
    {
        ...
        Err(e) => {
            ...
            let _ = ironclad_db::revenue_tax_tasks::release_revenue_tax_claim(
                &state.db,
                &opportunity_id,
            );
...
fn mark_tax_confirmed_with_metrics(
    ...
    if !updated {
        ...
        return Err(bad_request(format!(
            "revenue tax task for opportunity '{}' is not in_progress; may already be completed or failed",
            opportunity_id
        )));
    }
    ironclad_db::metrics::record_transaction_with_metadata(
