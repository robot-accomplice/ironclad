...
fn personality_state_from_workspace_loads_defaults() {
    ...
    assert!(!state.os_text.is_empty());
    assert!(state.os_text.contains("Roboticus"));
...
#[test]
fn personality_state_separates_os_and_firmware() {
    let dir = tempfile::tempdir().unwrap();
    personality::write_defaults(dir.path()).unwrap();

    let state = PersonalityState::from_workspace(dir.path());
    assert!(state.os_text.contains("iron-plated"));
    assert!(!state.os_text.contains("YOU MUST"));
    assert!(state.firmware_text.contains("YOU MUST"));
...
#[test]
fn voice_summary_in_os_text_when_non_default() {
    let dir = tempfile::tempdir().unwrap();
    let custom_os = personality::generate_os_toml("TestBot", "casual", "initiative", "developer");
    std::fs::write(dir.path().join("OS.toml"), &custom_os).unwrap();

    let state = PersonalityState::from_workspace(dir.path());
    assert!(state.os_text.contains("## Voice Profile"));
    assert!(state.os_text.contains("Formality: casual"));
    assert!(state.os_text.contains("Proactiveness: initiative"));
    assert!(state.os_text.contains("Domain: developer"));
}
